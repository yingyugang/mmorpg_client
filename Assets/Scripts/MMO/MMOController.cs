			...
			for (int i = 0; i < playerHandle.playerDatas.Length; i++) {
				if (playerHandle.playerDatas [i].playerId != mPlayerId) {
					if (!otherPlayers.ContainsKey (playerHandle.playerDatas [i].playerId)) {
						otherPlayers.Add (playerHandle.playerDatas [i].playerId, Instantiate (playerPrefab));
						otherPlayers [playerHandle.playerDatas [i].playerId].SetActive (true);
					}
					otherPlayers [playerHandle.playerDatas [i].playerId].transform.position = playerHandle.playerDatas [i].playerPosition;
					otherPlayers [playerHandle.playerDatas [i].playerId].transform.forward = playerHandle.playerDatas [i].playerForward;
					otherPlayers [playerHandle.playerDatas [i].playerId].GetComponent<SimpleRpgAnimator>().Action = playerHandle.playerDatas [i].action;
					otherPlayers [playerHandle.playerDatas [i].playerId].GetComponent<SimpleRpgAnimator> ().SetSpeed (playerHandle.playerDatas [i].animSpeed);
				}
			}
		}
