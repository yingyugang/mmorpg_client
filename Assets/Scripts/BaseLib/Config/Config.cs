...
namespace BaseLib
{
	public class Config<T> where T : struct, System.IConvertible
	{
        ...
        //自动校验字段个数是否和枚举匹配
		public ConfigTable getCfg<T1>(T key) where T1 : struct, System.IConvertible
        {
            ...
            {
				Debug.LogError("read config " + key +" failed!");
                return null;
            }
            return config;
        }
        ...
        private ConfigTable initCfg(T key)
        {
            ...
            if (config.init(strFileName))
            {
				...
				Debug.LogError("read config " + strFileName +" failed!");
            return null;
        }
	}
}
