...
namespace MMO
{
    public class TPSCameraController : BaseCameraController
    {
        ...
        void OnTouch(EventData eventData)
        {
            ...
            {
                if (eventData.currentTouch.touch.deltaPosition.x != 0)
                {
                    mDirect = Quaternion.AngleAxis(eventData.currentTouch.touch.deltaPosition.x * speed / 10, new Vector3(0, 1, 0)) * mDirect;
