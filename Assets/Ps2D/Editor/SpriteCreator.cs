...
namespace Ps2D
{
    ...
    public class SpriteCreator
    {
        ...
        public static GameObject CreateSprites(Layout layout, Layer startingLayer)
        {
            SpriteCreator creator = new SpriteCreator();
            creator.layout = layout;
            return creator.CreateSprites(startingLayer);
        }
        ...
        /// Create the sprites.
        /// </summary>
        public GameObject CreateSprites(Layer startingLayer)
        {
            // line up the ducks
        ...
        //add by hsw
        public void setChildZOffset(GameObject root)
        {
            if (root == null)
                return;
            float fMaxz = 0f;
            ...
            {
                Transform child = root.transform.GetChild(n);
                fMaxz = Mathf.Max(fMaxz, child.position.z);
            }

            if (fMaxz < 0f)
                return;
            float offset = -0.1f - fMaxz;
            for (int index = 0; index < count; index++)
            {
                Transform child = root.transform.GetChild(index);
                child.position = new Vector3(child.position.x,child.position.y,child.position.z + offset);
            }
        }
