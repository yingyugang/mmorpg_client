...
//#pragma target 5.0
//#pragma surface surf BlinnPhong addshadow vertex:disp tessellate:tessDistance nolightmap noambient 
...
void surf (Input IN, inout SurfaceOutput o) {
	...
	//o.Albedo = lerp(_DynReflColor.rgb,(refl.rgb*2.5*_DynReflColor.rgb*_DynReflColor.a),_DynReflColor.a);
	//o.Albedo = lerp(half3(0,0,0),o.Albedo,mask);
	//o.Albedo = lerp(o.Albedo,_DepthColorG.rgb,mask1);
	o.Alpha = 0.0;
...
void surf (Input IN, inout SurfaceOutput o) {
	...
	//half3 texFac = (v.normal * d *((1.0-z) * (1.0-s)));
	//half3 waveFac = (v.normal * (_WaveHeight*((1.0-z)*(1.0-s)))+lerp((h*(1.0-z)*(1.0-s)),0.0,(z)));
	//half3 shoreFac = (v.normal * (flowFactor*_WaveShoreHeight* (getflowmap.r-getflowmap.g)));
	//v.vertex.xyz += (v.normal * texFac* waveFac + shoreFac);
	...
	half waveFoam = (1.0-waveTex.b)*(getflowmap.r*0.5);//(getflowmap.g + getflowmap.r);
	...
	//fspread += (half3(lerp((1.0-_FoamHeight),1.0,((N6.y)*(_HeightFoamSpread))),0,0).r)*_HeightFoamAmount;
	//Add HeightFoam
	half3 texwave = tex2D(_WaveMap, IN.uv_FlowMap).rgb;
	half3 texflow = tex2D(_FlowMap, IN.uv_FlowMap).rgb;
...
void surf (Input IN, inout SurfaceOutput o) {
	...
	o.Albedo = half3(0,0,0);//lerp(half3(0,0,0),o.Albedo,mask3);
	o.Alpha = 0.0;
