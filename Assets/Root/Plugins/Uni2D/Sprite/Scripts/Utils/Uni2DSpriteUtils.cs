...
public static class Uni2DSpriteUtils 
{
	...
	public static Vector2[ ] BuildUVs( Texture2DContainer a_rTextureContainer, Vector2[ ] a_rRenderMeshUVs, Uni2DTextureAtlas a_rTextureAtlas )
	{
		...
		Vector2 f2UVOffset = Vector2.zero;	// Position (== min coords) of sprite UV rect in atlas. (0;0) if not atlased
		Vector2 f2UVRange = Vector2.zero;	// UV range (== max coords - position) in atlas. (1;1) if not atlased
