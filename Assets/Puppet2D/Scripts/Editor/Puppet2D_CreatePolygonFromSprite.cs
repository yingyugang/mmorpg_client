...
public class Puppet2D_CreatePolygonFromSprite : Editor {
    ...
    //add by hsw
    public GameObject Run(Transform transform, bool ReverseNormals, int triangleIndex,string strPath)
    {
        ...
        if (AssetDatabase.LoadAssetAtPath(strPath + "/" + transform.name + "_MESH.asset", typeof(Mesh)))
        {
            if (EditorUtility.DisplayDialog("Overwrite Asset?", "Do you want to overwrite the current Mesh & Material?", "Yes, Overwrite", "No, Create New Mesh & Material"))
            {
                //mf.mesh = AssetDatabase.LoadAssetAtPath("Assets/Puppet2D/Models/"+transform.name+"_MESH.asset",typeof(Mesh))as Mesh;
                string meshPath = (strPath + "/" + transform.name + "_MESH.asset");
                AssetDatabase.CreateAsset(mesh, meshPath);
                overwrite = true;
            }
            else
            {
                string meshPath = AssetDatabase.GenerateUniqueAssetPath(strPath +"/" + transform.name + "_MESH.asset");
                AssetDatabase.CreateAsset(mesh, meshPath);
            }
        }
        else
        ...
        if (overwrite)
        {
            mr.material = AssetDatabase.LoadAssetAtPath(strPath + "/" + "Materials/" + transform.name + "_MAT.mat", typeof(Material)) as Material;
        }
        else
        {
            ...
            Material newMat = new Material(Shader.Find("Unlit/Transparent"));
            //������Ŀ¼
            string folder = AssetDatabase.CreateFolder(strPath, "Materials");
            string materialPath = AssetDatabase.GenerateUniqueAssetPath(strPath + "/" + "Materials/" + transform.name + "_MAT.mat");
            AssetDatabase.CreateAsset(newMat, materialPath);
            mr.material = newMat;
        }
