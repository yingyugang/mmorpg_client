...
#region used to disable some effects type
		if(aType == _AnimType.Hit && (!heroResEffect.enableHitEffect || !enableHitEffect))
		{
			Debug.Log("enableHitEffect:" + heroResEffect.enableHitEffect);
			return;
		}
#endregion
	...
	IEnumerator Cast(EffectAttr ea)
	{
		...
		if(ea.effectTargetType==_EffectTargetType.Self)
		{
			if(ea.effectScopeType==_EffectScopeType.Single)
			{
				...
				{
					targets.Add(heroResEffect.gameObject);
				}
			}
			else
			{
...
//					GameObject castGo = Instantiate(ea.effectPrefab,castPos,Quaternion.identity)  as GameObject;
	...
	IEnumerator _Shoot(Transform target,EffectAttr ea,float delay = 0)
	{
		yield return new WaitForSeconds(delay);
//		Debug.Log("Shoot");
		Vector3 pos = ea.follow == null ? ea.position + heroResEffect.transform.position : ea.follow.position;
		...
		if (ea.effectPrefab != null)
		{
			...
			float t = 0;
			float x = Random.Range (0,(target.position.x - startPos.x) * 2);
			float y = Random.Range (0,(target.position.y - startPos.y) * 2);
			Vector2 controllPos = new Vector2 (x + startPos.x,y+ startPos.y);
			if(ea.effectShootTrack==_EffectShootTrack.Projection)
			{
				controllPos = new Vector2((startPos.x + target.position.x)/2,Mathf.Max(startPos.y,target.position.y) + Random.Range(5,20));
			}
			...
			if(ea.effectShootTrack==_EffectShootTrack.Missle)
			{
				while(t<1)
				{
					...
					shootObject.transform.position = Vector2.Lerp(startPos,target.position,t);
					yield return null;
				}
			}
			else
			{
				while(t<1)
				{
					...
					{
						shootObject.transform.position = Curve.Bezier2(startPos,controllPos,(Vector2)target.position,t);
					}
					yield return null;
				}
			}
		...
		if(ea.hitEffectPrefab!=null)
		{
			GameObject hitGo = PoolManager.SingleTon().Spawn(ea.hitEffectPrefab,target.position,Quaternion.Euler(ea.rotation)) as GameObject;
			PoolManager.SingleTon().UnSpawn(3,hitGo);
		}
		//Debug.Log("Destroy:" + shootObject);
