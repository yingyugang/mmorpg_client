...
//                     }
                    ...
                    {
                        //交换
                        DataManager.getModule<DataHero>(DATA_MODULE.Data_Hero).ChangeTeamerById(ddg.idGround, idHero, pe.nCurTeamId);
                        DataManager.getModule<DataHero>(DATA_MODULE.Data_Hero).ChangeTeamerById(pe.nDragGround, nID, pe.nCurTeamId);

                        if (DataManager.getModule<DataHero>(DATA_MODULE.Data_Hero).dicHeroTeams[pe.nCurTeamId + 1].leaderPos == ddg.idGround + 1)
                        {
                            DataManager.getModule<DataHero>(DATA_MODULE.Data_Hero).dicHeroTeams[pe.nCurTeamId + 1].leaderPos = pe.nDragGround + 1;
                        }
                        else if (DataManager.getModule<DataHero>(DATA_MODULE.Data_Hero).dicHeroTeams[pe.nCurTeamId + 1].leaderPos == pe.nDragGround + 1)
                        {
                            DataManager.getModule<DataHero>(DATA_MODULE.Data_Hero).dicHeroTeams[pe.nCurTeamId + 1].leaderPos = ddg.idGround + 1;
                        }
                    ...
                    {
                        DataManager.getModule<DataHero>(DATA_MODULE.Data_Hero).ChangeTeamerById(ddg.idGround, idHero, pe.nCurTeamId);
                        DataManager.getModule<DataHero>(DATA_MODULE.Data_Hero).DelTeamerById(pe.nDragGround, idHero, pe.nCurTeamId);

                        if (DataManager.getModule<DataHero>(DATA_MODULE.Data_Hero).dicHeroTeams[pe.nCurTeamId + 1].leaderPos == pe.nDragGround + 1)
                        {
                            DataManager.getModule<DataHero>(DATA_MODULE.Data_Hero).dicHeroTeams[pe.nCurTeamId + 1].leaderPos = ddg.idGround + 1;
                        }
