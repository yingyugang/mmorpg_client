...
public class AlineOnChild : MonoBehaviour
{
	/// <summary>
	...
	{
        CycleGrid cg = gameObject.GetComponent<CycleGrid>();
        if (cg != null)
        {
            cg.ResetCyclePosition();
        }
    ...
    public void SetPartnerEditTeamID()
    {
    ...
    Vector3 mScrollViewPosition = new Vector3();
    ...
    public void ReHide()
    {
        ...
        if (mScrollViewPosition == mScrollView.transform.localPosition)
        {
            return;
        }

        mScrollViewPosition = mScrollView.transform.localPosition;
        ...
        for (int i = 0, imax = trans.childCount; i < imax; ++i)
        {
            ...
            Vector3 cp = panelTrans.InverseTransformPoint(t.position);
            Vector3 cc = panelTrans.InverseTransformPoint(panelCenter);
            Vector3 localOffset = cp - cc;


            BoxCollider bc = t.gameObject.GetComponent<BoxCollider>();
            Vector2 panelViewSize = mScrollView.panel.GetViewSize();

            if (!mScrollView.canMoveHorizontally)
            {
                localOffset.x = 0f;

                if ((panelViewSize.y + bc.size.y) * 0.5 > Math.Abs(localOffset.y))
                {
            ...
            {
                if ((panelViewSize.x + bc.size.x) * 0.5 > localOffset.x)
                {
                    t.gameObject.SetActive(true);
                }
                else
