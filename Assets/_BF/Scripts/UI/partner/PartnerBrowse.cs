...
public class PartnerBrowse : MonoBehaviour
{
    ...
    void onDetail(GameObject go)
    {
        ...
        pd.isSummonUse = false;
        pd.ShowHeroDetail(id, gameObject);

        //DataManager.getModule<DataHero>(DATA_MODULE.Data_Hero).MainShowHeroDetailById(id);
    }
    ...
    public void ShowHeroBrowse()
    {
        ...
        for (int i = 0; i < (nCount / 4 + 1); ++i)
        {
            ...
            for (int j = i * 4; j < (i * 4 + 4); ++j)
            {
                ...
                HeroInfo hi = DataManager.getModule<DataHero>(DATA_MODULE.Data_Hero).heroInfoList[j];
