...
public class ArenaTeam : MonoBehaviour
{
	...
	void Start ()
    {
        ...
        step1 = PanelTools.findChild(transform.parent.gameObject, "step1");
        ...
        if (pveBtn != null)
        {
            UIEventListener.Get(pveBtn.gameObject).onClick = onPVE;
        } 
    ...
    void onReturn(GameObject go)
    {
        ...
        DataManager.getModule<DataArena>(DATA_MODULE.Data_Arena).SendHeroFormationArenaMsg(nCurTeamId);
    }

    void onPVE(GameObject go)
    {
        gameObject.SetActive(false);
        transform.parent.gameObject.SetActive(false);

        DataManager.getModule<DataHero>(DATA_MODULE.Data_Hero).ArenaShowTeamEdit();
    }
    ...
    protected void ChangeState()
    {
        ...
        if (nState == 2)
        {
            ...
            {
                ArenaDragPveItem cdi = item.GetComponent<ArenaDragPveItem>();
                cdi.enabled = true;
    ...
    void onGround(GameObject go)
    {
        ...
        if (ddg != null)
        {
            ...
            SelectArenaHero sah = selectTeamer.GetComponent<SelectArenaHero>();
            sah.idGround = ddg.idGround;
    ...
    void onItem(GameObject go)
    {
        ...
        if (nState == 1)
        {
            ...
            DataManager.getModule<DataArena>(DATA_MODULE.Data_Arena).dicArenaTeams[nCurTeamId + 1].leaderPos = ddg.idGround + 1;
        ...
        else
        {
            ...
            DragDropGround ddg = go.transform.parent.GetComponent<DragDropGround>();
            ChangeArenaHero cc = changeTeamer.GetComponent<ChangeArenaHero>();
            cc.idHero = id;
    ...
    public void InitShowTeam()
    {
        ...
        int nCount = DataManager.getModule<DataArena>(DATA_MODULE.Data_Arena).dicArenaTeams.Count;
        ...
        if (DataManager.getModule<DataArena>(DATA_MODULE.Data_Arena).nPvpTeamId == 1)
        {
            nCurTeamId = 1;
        }
        else
    ...
    public void ShowTeamHero()
    {
        ...
        DataManager.getModule<DataArena>(DATA_MODULE.Data_Arena).dicArenaTeams.TryGetValue(nCurTeamId + 1, out curTeamInfo);
    ...
    public void SetCurTeam()
    {
        ...
        teamLabel.text = (nCurTeamId + 1).ToString() + "/2";
    }
