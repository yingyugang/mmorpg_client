...
public class ArenaDragPveItem : MonoBehaviour
{
    ...
    {
        //隐藏属性信息记录位置
        ArenaTeam ct = arenaTeam.GetComponent<ArenaTeam>();
        if (ct.nState != 2)
        { 
            return;
        }
        ...
        if (cloneOnDrag)
        {
            ...
            ArenaDragPveItem item = clone.GetComponent<ArenaDragPveItem>();
            item.Start();
        ...
        if (surface != null)
        {
            ...
            if (ddg != null)
            {
                ...
                if (idLable != null && idHeroLable != null)
                {
                    ...
                    {
                        //交换
                        DataManager.getModule<DataArena>(DATA_MODULE.Data_Arena).ChangeTeamerById(ddg.idGround, idHero, ct.nCurTeamId);
                        DataManager.getModule<DataArena>(DATA_MODULE.Data_Arena).ChangeTeamerById(ct.nDragGround, nID, ct.nCurTeamId);

                        if (DataManager.getModule<DataArena>(DATA_MODULE.Data_Arena).dicArenaTeams[ct.nCurTeamId + 1].leaderPos == ddg.idGround + 1)
                        {
                            DataManager.getModule<DataArena>(DATA_MODULE.Data_Arena).dicArenaTeams[ct.nCurTeamId + 1].leaderPos = ct.nDragGround + 1;
                        }
                        else if (DataManager.getModule<DataArena>(DATA_MODULE.Data_Arena).dicArenaTeams[ct.nCurTeamId + 1].leaderPos == ct.nDragGround + 1)
                        {
                            DataManager.getModule<DataArena>(DATA_MODULE.Data_Arena).dicArenaTeams[ct.nCurTeamId + 1].leaderPos = ddg.idGround + 1;
                        }
                    ...
                    {
                        DataManager.getModule<DataArena>(DATA_MODULE.Data_Arena).ChangeTeamerById(ddg.idGround, idHero, ct.nCurTeamId);
                        DataManager.getModule<DataArena>(DATA_MODULE.Data_Arena).DelTeamerById(ct.nDragGround, idHero, ct.nCurTeamId);

                        if (DataManager.getModule<DataArena>(DATA_MODULE.Data_Arena).dicArenaTeams[ct.nCurTeamId + 1].leaderPos == ct.nDragGround + 1)
                        {
                            DataManager.getModule<DataArena>(DATA_MODULE.Data_Arena).dicArenaTeams[ct.nCurTeamId + 1].leaderPos = ddg.idGround + 1;
                        }
