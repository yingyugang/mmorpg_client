...
public class ArenaRankList : MonoBehaviour {
	...
	void Start () 
    {
        ...
        PanelTools.setBtnFunc(transform, "Title/friendBtn", onFriend);
	}
    ...
    void onFriend(GameObject obj)
    {
        gameObject.SetActive(false);
        friendRank.SetActive(true);

        FriendRankList frl = friendRank.GetComponent<FriendRankList>();
        frl.pageIndex = pageIndex;

        DataManager.getModule<DataArena>(DATA_MODULE.Data_Arena).SendArenaFriendRankingMsg();
    }
    ...
    public void ShowRankList()
    {
        ...
        int nCount = DataManager.getModule<DataArena>(DATA_MODULE.Data_Arena).totalRankList.theList.Count;

        for (int i = 0; i < nCount; ++i)
        {
            ArenaRankUser aru = DataManager.getModule<DataArena>(DATA_MODULE.Data_Arena).totalRankList.theList[i];
