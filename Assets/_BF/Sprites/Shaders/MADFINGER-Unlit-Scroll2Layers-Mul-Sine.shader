...
Properties {
	...
	_DetailTex ("1nd layer (RGB)", 2D) = "white" {}
	_DetailTex0 ("2nd layer (RGB)", 2D) = "white" {}
...
//	Blend One One
	...
	Pass {
		...
		v2f vert (appdata_full v)
		{
			...
			o.uv.xy = TRANSFORM_TEX(v.texcoord.xy,_DetailTex) + frac(float2(_ScrollX, _ScrollY) * _Time);
			o.uv.zw = TRANSFORM_TEX(v.texcoord.xy,_DetailTex0) + frac(float2(_Scroll2X, _Scroll2Y) * _Time);
			...
			o.color.xy = TRANSFORM_TEX(v.texcoord.xy,_MainTex);
			return o;
		}		
		...
		{
			fixed4 o;
			fixed4 tex = tex2D (_DetailTex, i.uv.xy);
			fixed4 tex2 = tex2D (_DetailTex0, i.uv.zw);
			float4 tex3 = tex2D (_MainTex, i.color.xy);
			o = tex * tex2;
			tex3.xyz = o.xyz;
			return tex3 + o;
		}
